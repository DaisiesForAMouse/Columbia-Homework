...
% T(p + q) &= T(\sum_{i=1}^np_ix^i + \sum_{i=1}^nq_ix^i) \\
% &= \sum_{i=1}^np_i(x + 1)^i + \sum_{i=1}^nq_i(x+1)^i
% &= T
...
% To conclude that $\mathcal{F}(\Zg, \R)$ and $\mathcal{F}(\R, \R)$ are infinite
% dimensional, we have from results in class that for a vector space of dimension
% $n$, any collection of $n$ independent vectors in the vector space form a basis.
